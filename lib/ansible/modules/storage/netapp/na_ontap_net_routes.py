...
options:
  ...
  metric:
    description:
    - Specify the route metric. If this field is not provided the default will be set to 30
    required: false
'''
...
class NetAppOntapNetRoutes(object):
    ...
    def delete_net_route(self):
        ...
        try:
            ...
            self.module.fail_json(msg='Error deleting net route: %s' % (to_native(error)),
                                  exception=traceback.format_exc())
    ...
    def modify_net_route(self):
        ...
        if self.new_metric:
            ...
            route_obj.add_new_child("metric", self.metric)
        # if anything goes wrong creating the new net route, restore the old one
        try:
            self.server.invoke_successfully(route_obj, True)
        except:
