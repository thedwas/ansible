'''Module to create/delete job schedule cron'''
#!/usr/bin/python
...
options:
  ...
  job_minutes:
    ...
    - The minute(s) of each hour when the job should be run.
      ...
      -1 represents all minutes and only supported for cron schedule create and modify.
      Range is [-1..59]
    required: false
'''
...
class NetAppONTAPJob(object):
    ...
    def get_job_schedule(self):
        ...
        job_schedule_info = netapp_utils.zapi.NaElement('job-schedule-cron-info')
        ...
        if result.get_child_by_name('num-records') and \
            ...
            return_value = {
                ...
                'job_minutes': job_exists_info.get_child_by_name('job-schedule-cron-minute').get_child_content('cron-minute')
            }
        return return_value
    ...
    def create_job_schedule(self):
        ...
        job_schedule_create = netapp_utils.zapi.NaElement.create_node_with_children(
            'job-schedule-cron-create', **{'job-schedule-name': self.name})
        job_schedule_create.add_node_with_children('job-schedule-cron-minute',
    ...
    def delete_job_schedule(self):
        ...
        job_schedule_delete = netapp_utils.zapi.NaElement.create_node_with_children(
            'job-schedule-cron-destroy', **{'job-schedule-name': self.name})
        try:
    ...
    def modify_minute_job_schedule(self):
        ...
        """
        job_schedule_modify_minute = netapp_utils.zapi.NaElement.create_node_with_children(
            'job-schedule-cron-modify', **{'job-schedule-name': self.name})
        job_schedule_modify_minute.add_node_with_children('job-schedule-cron-minute',
                                                   **{'cron-minute': str(self.job_minutes)})
        try:
